  ...
  }
{
  match op {
    ...
    Pop => if list.is_empty(stack) {
      Err("underflow")
    } else {
      Ok(list.tail(stack))
    },
    Add => if list.is_empty(stack) {
      Err("underflow")
    } else {
      let top1 := match list.head(stack) {
        Some(v) => v,
        None => 0,
      }
      ...
      if list.is_empty(rest) {
        ...
        let top2 := match list.head(rest) {
          Some(v) => v,
          None => 0,
        }
        let rest2 := list.tail(rest)
        Ok(list.cons(top1 + top2, rest2))
      }
    },
    Mul => if list.is_empty(stack) {
      Err("underflow")
    } else {
      let top1 := match list.head(stack) {
        Some(v) => v,
    ...
    Dup => if list.is_empty(stack) {
      ...
      let top := match list.head(stack) {
        Some(v) => v,
        None => 0,
      }
      Ok(list.cons(top, stack))
    },
    Swap => if list.is_empty(stack) {
      Err("underflow")
    } else {
      let top1 := match list.head(stack) {
        Some(v) => v,
      ...
      if list.is_empty(rest) {
        ...
        let rest2 := list.tail(rest)
        Ok(list.cons(top2, list.cons(top1, rest2)))
      }
    },
  }
}
...
fn run(ops :: List[Op]) -> Result[List[Int], Str]
  examples {
    ...
    run([Push(1), Pop, Add]) => Err("underflow")
  }
{
