  ...
  }
{
  ...
  fn go(xs :: List[Int], current :: Run, acc :: List[Run]) -> List[Run] {
    if list.is_empty(xs) {
      ...
      match current {
        Segment(count, value) =>
          if h == value {
            go(t, Segment(count + 1, value), acc)
          } else {
            go(t, Segment(1, h), list.cons(current, acc))
          },
      }
    }
  }
  ...
  }
{
  ...
  list.fold(runs, [], fn (acc :: List[Int], run :: Run) -> List[Int] {
    list.concat(acc, list.reverse(expand(run, [])))
  })
}
