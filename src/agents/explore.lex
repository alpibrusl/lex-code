...
fn ollama_agent() -> [env] ag.AgentLoop {
  let base := { name: "explore", goal: ep.system(), model: prov.ollama(providers.ollama_model()), provider: providers.ollama_local(), tools: tools.explore_dynamic_tools(), options: { temperature: None, top_p: None, max_steps: Some(20), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.explore_permission())
}

fn litellm_agent() -> [env] ag.AgentLoop {
  let base := { name: "explore", goal: ep.system(), model: prov.make_model_ref("litellm", tools.litellm_model()), provider: providers.litellm(), tools: tools.explore_dynamic_tools(), options: { temperature: None, top_p: None, max_steps: Some(20), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.explore_permission())
}
...
fn opencode_agent() -> [env] ag.AgentLoop {
  let base := { name: "explore", goal: ep.system(), model: prov.make_model_ref("opencode", tools.opencode_model()), provider: providers.opencode_go(), tools: tools.tools_for_spec(rules.explore_permission()), options: { temperature: None, top_p: None, max_steps: Some(20), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.explore_permission())
}
