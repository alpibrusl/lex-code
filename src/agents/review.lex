...
fn litellm_agent() -> [env] ag.AgentLoop {
  let base := { name: "review", goal: rvp.system(), model: prov.make_model_ref("litellm", tools.litellm_model()), provider: providers.litellm(), tools: [], options: { temperature: None, top_p: None, max_steps: Some(3), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.review_permission())
}
...
fn opencode_agent() -> [env] ag.AgentLoop {
  let base := { name: "review", goal: rvp.system(), model: prov.make_model_ref("opencode", tools.opencode_model()), provider: providers.opencode_go(), tools: tools.tools_for_spec(rules.review_permission()), options: { temperature: None, top_p: None, max_steps: Some(25), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.review_permission())
}
