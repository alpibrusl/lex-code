...
fn litellm_agent() -> [env] ag.AgentLoop {
  let base := { name: "plan", goal: pp.system(), model: prov.make_model_ref("litellm", tools.litellm_model()), provider: providers.litellm(), tools: [], options: { temperature: None, top_p: None, max_steps: Some(3), max_tokens: None }, permission_spec: None }
  ag.with_permission_gate(base, rules.plan_permission())
}
