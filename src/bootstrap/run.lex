...
fn main() -> [io, net, llm, proc, sql, time] Nil {
  ...
  let phases := [{ name: "impl", task: task, mode: sess.Build }, { name: "spec", task: str.concat("Write lex-spec Spec for list.zip: ", task), mode: sess.Spec }, { name: "test", task: str.concat("Write unit tests for list.zip: ", task), mode: sess.Test }, { name: "review", task: str.concat("Review the list.zip implementation and tests: ", task), mode: sess.Review }]
