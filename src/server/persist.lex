...
fn log_turn_start(log :: trail_log.Log, parent :: Option[Str], user_text :: Str) -> [sql, time] Result[ev.Event, Str] {
  let payload := str.concat("{\"role\":\"user\",\"text\":\"", str.concat(user_text, "\"}"))
  trail_log.append(log, kinds.llm_step(), parent, payload)
}

fn log_turn_done(log :: trail_log.Log, parent :: Option[Str], assistant_text :: Str) -> [sql, time] Result[ev.Event, Str] {
