...
fn get_env_w(key :: Str, fallback :: Str) -> [env] Str {
  match env.get(key) {
    None    => fallback,
    Some(v) => if str.is_empty(v) { fallback } else { v },
  }
}
...
fn mode_from_str(s :: Str) -> sess.AgentMode {
  if s == "plan" { Plan } else {
  if s == "explore" { Explore } else {
  if s == "refactor" { Refactor } else {
  if s == "spec" { Spec } else {
  if s == "test" { Test } else {
  if s == "review" { Review } else {
  Build } } } } } }
}
...
fn with_cors(r :: resp.Response) -> resp.Response {
  resp.with_header(resp.with_header(resp.with_header(r,
...
fn handle_a2a_body(body :: Str) -> [env, io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] resp.Response {
  match jv.parse(body) {
    ...
    Ok(j)  => {
      ...
      let prov     := if str.is_empty(provider) { "anthropic" } else { provider }
        ...
        match sess.new_session_with_provider(sid, mode, prov) {
          ...
          Ok(session) => {
            ...
            resp.json(str.join([
              ...
              ",\"result\":{\"session_id\":", jv.stringify(JStr(sid)),
              ",\"steps\":", steps_json, "}}"
            ], ""))
          },
        }
...
fn build_static_router(web_dir :: Str) -> router.Router {
  let r0 := router.new()
  let r1 := router.route_effectful(r0, "GET", "/",
