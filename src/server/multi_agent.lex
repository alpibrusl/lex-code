...
fn worker_handler(state :: WorkerState, msg :: WorkerMsg)
  -> [env, net, llm, io, proc, sql, fs_write, time] (WorkerState, List[d.Step]) {
  match msg {
    Execute(task) =>
      match sess.new_session_with_provider("worker", state.mode, state.provider_tag) {
        Err(_)      => (state, []),
        Ok(session) => {
...
fn run_parallel(task :: Str, provider_tag :: Str)
  -> [env, concurrent, net, llm, io, proc, sql, fs_write, time] MultiResult {
...
fn run_impl_then_test(task :: Str, provider_tag :: Str)
  -> [env, net, llm, io, proc, sql, fs_write, time] MultiResult {
...
fn run_impl_then_spec_then_test(task :: Str, provider_tag :: Str)
  -> [env, concurrent, net, llm, io, proc, sql, fs_write, time] List[d.Step] {
  match sess.new_session_with_provider("impl", sess.Build, provider_tag) {
    Err(_) => [],
    Ok(impl_sess) => {
      let impl_result := sess.run_turn_with_provider(impl_sess, task, provider_tag)
      ...
      match sess.new_session_with_provider("spec", sess.Spec, provider_tag) {
        ...
        Ok(spec_sess) => {
          ...
          list.concat(impl_result.steps,
            list.concat(spec_result.steps,
              list.concat(test_steps, rev_steps)))
        }
      }
    }
  }
