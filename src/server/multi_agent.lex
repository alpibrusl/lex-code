...
fn run_parallel(task :: Str, provider_tag :: Str) -> [env, concurrent, net, llm, io, proc, sql, fs_write, time] MultiResult {
  let impl_state := { mode: Build, provider_tag: provider_tag }
  let test_state := { mode: Test, provider_tag: provider_tag }
...
fn run_impl_then_test(task :: Str, provider_tag :: Str) -> [env, net, llm, io, proc, sql, fs_write, time] MultiResult {
  match sess.new_session_with_provider("impl", Build, provider_tag) {
    ...
    Ok(impl_sess) => {
      ...
      match sess.new_session_with_provider("test", Test, provider_tag) {
...
fn run_impl_then_spec_then_test(task :: Str, provider_tag :: Str) -> [env, concurrent, net, llm, io, proc, sql, fs_write, time] List[d.Step] {
  match sess.new_session_with_provider("impl", Build, provider_tag) {
    ...
    Ok(impl_sess) => {
      ...
      match sess.new_session_with_provider("spec", Spec, provider_tag) {
        ...
        Ok(spec_sess) => {
          ...
          let test_actor := conc.spawn({ mode: Test, provider_tag: provider_tag }, worker_handler)
          let rev_actor := conc.spawn({ mode: Review, provider_tag: provider_tag }, worker_handler)
