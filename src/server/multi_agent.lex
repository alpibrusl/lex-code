...
fn run_task(task :: Str, mode :: sess.AgentMode, provider_tag :: Str) -> [env, net, llm, io, proc, sql, fs_write, time, approval] List[d.Step] {
...
fn run_parallel(task :: Str, provider_tag :: Str) -> [env, concurrent, net, llm, io, proc, sql, fs_write, time, approval] MultiResult {
  let pairs := [(task, Build), (str.concat("Write unit tests for: ", task), Test)]
  let results := list.par_map(pairs, fn (pair :: (Str, sess.AgentMode)) -> [env, net, llm, io, proc, sql, fs_write, time, approval] List[d.Step] {
    match pair {
...
fn run_impl_then_test(task :: Str, provider_tag :: Str) -> [env, net, llm, io, proc, sql, fs_write, time, approval] MultiResult {
...
fn run_impl_then_spec_then_test(task :: Str, provider_tag :: Str) -> [env, concurrent, net, llm, io, proc, sql, fs_write, time, approval] List[d.Step] {
  let impl_steps := run_task(task, Build, provider_tag)
  ...
  let parallel_results := list.par_map(parallel_tasks, fn (pair :: (Str, sess.AgentMode)) -> [env, net, llm, io, proc, sql, fs_write, time, approval] List[d.Step] {
    match pair {
      (t, mode) => run_task(t, mode, provider_tag),
    }
  })
