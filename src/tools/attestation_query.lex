  ...
  match util.field_str(args, "fn_name") {
    ...
    Some(fn_name) =>
      ...
      match proc.spawn("lex", cmd_args) {
        ...
        Ok(out)  =>
          let result :=
            if str.is_empty(out.stdout) then
...
fn tool() -> t.Tool {
  t.define(
    "attestation_query",
    "List attestations on a function from lex-trail. Shows who attested, when, and the attestation chain.",
    params(),
    execute)
}
