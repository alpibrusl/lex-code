...
fn params() -> s.ModelSchema {
  { title: "LexStoreApplyArgs",
    description: "Apply an AST-level Operation to the lex-store",
    fields: [
      s.required_str("operation", []),
    ] }
}
...
fn tool() -> t.Tool {
  t.define(
    "lex_store_apply",
    "Apply an AST-level Operation (JSON) to the lex-store. Preferred over text-edit for structural changes.",
    params(),
    execute)
}
