...
fn params() -> s.ModelSchema {
  { title: "EditArgs",
    description: "Edit a file by exact string replacement. old_str must appear exactly once.",
    fields: [
      s.required_str("path", []),
      s.required_str("old_str", []),
      s.required_str("new_str", []),
    ] }
}
...
fn tool() -> t.Tool {
  t.define(
    "edit",
    "Edit a file by replacing old_str with new_str. old_str must be unique in the file.",
    params(),
    execute)
}
