...
fn execute(args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] {
  match util.field_str(args, "op_id") {
    ...
    Some(id) => {
      # With `ollama_model`, regenerate via the local Ollama daemon and
      # record the Replay verdict; without it, print the replay request
      # (target signature, recorded intent, parent program) for an
