...
fn params() -> s.ModelSchema {
  { title: "VcsBranchPeekArgs",
    description: "Read another branch's ops without switching to it.",
    fields: [
      s.required_str("branch", []),
      s.optional(s.required_str("vs", [])),
      s.optional(s.required_str("since_fork", [])),
    ] }
}
...
fn tool() -> t.Tool {
  t.define(
    "vcs_branch_peek",
    "Read another branch's operations without switching to it. Use since_fork=true to see only divergence from a common ancestor.",
    params(),
    execute)
}
