...
fn params() -> s.ModelSchema {
  { title: "VcsBranchOverlayArgs",
    description: "Preview what a merge would produce without persisting anything.",
    fields: [
      s.required_str("other_branch", []),
      s.optional(s.required_str("on_branch", [])),
    ] }
}
...
fn tool() -> t.Tool {
  t.define(
    "vcs_branch_overlay",
    "Preview what the current (or on_branch) branch would look like if other_branch were merged. Pure read — nothing is persisted.",
    params(),
    execute)
}
