    ...
    VArr(items) => {
      let inner := list.fold(items, "", fn (acc :: Str, item :: Val) -> Str {
        ...
        if str.is_empty(acc) {
          ...
          str.concat(str.concat(acc, ", "), item_str)
        }
      })
      ...
      let inner := list.fold(fields, "", fn (acc :: Str, field :: (Str, Val)) -> Str {
        match field {
          (k, v) => {
            ...
            let field_str := str.concat(str.concat(key_str, ": "), val_str)
            if str.is_empty(acc) {
              field_str
            } else {
              str.concat(str.concat(acc, ", "), field_str)
            }
          }
        }
      })
  ...
  match v {
    VObj(fields) => {
      list.fold(fields, None, fn (acc :: Option[Val], field :: (Str, Val)) -> Option[Val] {
        match acc {
          Some(_) => acc,
          None    => match field { (k, v) => if k == key { Some(v) } else { None } },
        }
      })
    },
    _           => None,
