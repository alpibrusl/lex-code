  ...
  }
{
  match v {
    ...
    VArr(arr)     => {
      let items := list.fold(arr, [], fn (acc :: List[Str], item :: Val) -> List[Str] {
    ...
    VObj(obj)     => {
      let items := list.fold(obj, [], fn (acc :: List[Str], kv :: (Str, Val)) -> List[Str] {
        match kv {
          (k, v) => list.cons(str.concat("\"", str.concat(k, str.concat("\": ", stringify(v)))), acc)),
        }
      });
      let reversed := list.reverse(items);
      let joined := str.join(reversed, ", ");
