  ...
  }
{
  match expr {
    ...
    Var(name) => match list.fold(env, None, fn (acc :: Option[Int], binding :: (Str, Int)) -> Option[Int] {
      match acc {
        Some(v) => Some(v),
        None => match binding {
          (n, v) => if n == name {
  ...
  }
{
  match expr {
    Lit(n) => int.to_str(n),
    Add(e1, e2) => str.concat("(", str.concat(str.concat(to_str(e1), " + "), to_str(e2)), ")"),
    Mul(e1, e2) => str.concat("(", str.concat(str.concat(to_str(e1), " * "), to_str(e2)), ")"),
