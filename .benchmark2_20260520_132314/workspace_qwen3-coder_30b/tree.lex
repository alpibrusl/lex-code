...
# Custom type for binary search tree
  ...
  }
{
  match t {
    ...
    Node(v, l, r)  => if n < v { Node(v, insert(l, n), r) } else { Node(v, l, insert(r, n)) },
  }
}
  ...
  }
{
  match t {
    Leaf                    => false,
    Node(v, l, r)  => if n == v { true } else { if n < v { contains(l, n) } else { contains(r, n) } },
  }
}
  ...
  }
{
  match t {
    Leaf           => [],
    Node(v, l, r)  => list.concat(to_sorted_list(l), list.concat([v], to_sorted_list(r))),
  }
}