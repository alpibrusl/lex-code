  ...
  }
{
  match v {
    ...
    VArr(arr)     => {
      let arr_str := list.fold(arr, "", fn (acc :: Str, item :: Val) -> Str {
        if str.is_empty(acc) { stringify(item) } else { str.concat(str.concat(acc, ", "), stringify(item)) }
      })
      str.concat("[", str.concat(arr_str, "]"))
    },
    VObj(obj)     => {
      let obj_str := list.fold(obj, "", fn (acc :: Str, kv :: (Str, Val)) -> Str {
        match kv {
          (k, v) => {
            let item_str := str.concat(str.concat("\"", str.concat(k, "\": ")), stringify(v))
            if str.is_empty(acc) { item_str } else { str.concat(str.concat(acc, ", "), item_str) }
          }
        }
      })
